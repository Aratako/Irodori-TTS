...
rem
rem 使い方:
rem   1. このファイルを _IRODORI_LOCAL_CONFIG.bat にコピーします。
rem   2. 必要に応じて下の値を書き換えます。
...
rem 0 の場合は公式配布状態に近く、HF repo ID をそのまま使います。
set "IRODORI_TTS_OFFLINE=0"

rem 公式デフォルトのモデルとCodecのrepo IDです。
rem オフライン運用する場合は、ローカルの model.safetensors / weights.pth へのパスに変更してください。
